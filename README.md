...
_ [x] 点赞功能就是post了一次，其中比较新颖的是就是这个。
_ [x] 发送评论这里，其实比较新颖的是json格式和字典格式是不同的，在'Content-Type'= 'application/json'其中就必须先将字典dump成json才能发送表单。
_ [x] 获取评论这里是通过一次while和cnt来控制获取全部的评论。
